...
async fn wallet_accept_message(world: &mut DlcLinkWorld, accept_message: String, context: String) {
    ...
    world.collected_responses.insert(context.clone(), res.await.unwrap());
}

#[when(expr="creating an offer request '{word}' with uuid {word}, accept_collateral: {u64} and offer_collateral: {u64}")]
async fn create_offer(world: &mut DlcLinkWorld, context: String, uuid: String, accept_collateral: CustomU64, offer_collateral: CustomU64) {
